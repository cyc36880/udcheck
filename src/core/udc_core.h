...
typedef struct _udc_pack_init
{
    ...
    uint16_t transmit_buffer_size;
    uint8_t is_transmit_buffer_dynamic : 1;
    uint8_t is_receive_buffer_dynamic : 1;
} udc_pack_init_t;
 ...
 *  @note If it is inconsistent with the default number of digits of the check value, Modifications are required "UDC_INSPECT_BIT_SIZE".
 * @param pack             Package pointer
 ...
 * @param pack                Package pointer
 * @param receive_or_transmit 0:receive 1:transmit
 * @return                    buffer
 */
 ...
 * @return                   0:success -1:failed
 */
int udc_pack_get_first_obj(const udc_pack_t *pack, uint8_t revive_or_transmit, udc_obj_t *obj);
 ...
 * @return                   0:success -1:failed
 */
int udc_pack_get_next_obj(const udc_pack_t *pack, uint8_t revive_or_transmit, udc_obj_t *base_obj, udc_obj_t *next_obj);
 ...
 * @return                   0:success -1:failed
 */
int udc_pack_get_obj(const udc_pack_t *pack, uint8_t revive_or_transmit, uint8_t id, udc_obj_t *obj);
